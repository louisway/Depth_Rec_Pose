...
public:
    ...
    void upload_frame(frame&& f, cv::Mat &mask)
    {
    ...
    {
        texture.show(stream_rect, get_stream_alpha(), data);
...
public:
    ...
    {
        auto stream_type = f.get_stream_type();
        streams[stream_type].upload_frame(std::move(f), mask);
    }
...
/*void GetImageFile(const uchar* image_file,
                  mx_float* image_data, const int channels) {
    int rows = NETWORK_INPUT;
    int cols = NETWORK_INPUT;
    ...
    for (int i = 0; i < rows; i++) {
        const uchar* data = image_file + i * cols * 3;
...
struct param_p
{
    ...
    bool *terminated;
    int factor;
    param_p(std::string model_file, std::string trained_file,std::string pose_model_file, std::string pose_trained_file,bool *term, int factors, bool isGpu):mask(480,640,CV_8UC1,cv::Scalar(0)){
  //:classifier(model_file, trained_file, 0, 0),pose_classifier(pose_model_file,pose_trained_file,0,0)
         //Classifier classifier(model_file, trained_file, 0, 0);
         //pose_Classifier pose_classifier(pose_model_file,pose_trained_file,0,0);
         classifier = new Classifier(model_file,trained_file,isGpu,0);
         pose_classifier = new pose_Classifier(pose_model_file,pose_trained_file,isGpu,0);
         terminated = term;
         factor = factors;
    }
    ~param_p()
    {
         delete classifier;
         delete pose_classifier;
         delete terminated;
    }
    /*
    param_p& operator=(param_p &B)
    {
        classifier = B->classifier;
        pose_classifier = B->pose_classifier;
        img_depth = B->img_depth;
        data = B->data;
        out_width = B->out_width;
        out_height = B->out_height;
        min_area = B->min_area;
    }*/
};

void* classify(void* params) 
{
    //Caffe::set_mode(Caffe::CPU);
    struct param_p* args_p = ((struct param_p*) params);
    
    //std::cout << img_depth << std::endl;
    std::vector<cv::Rect> Record;
    std::vector<int> class_result;
    std::vector<float> probs;
    
    predict(args_p->img_depth, args_p->mask, args_p->classifier, args_p->pose_classifier,args_p->out_width,args_p->out_height, args_p->min_area, Record, class_result, probs,args_p->factor);
    //predict(((struct param_p*) params)->img_depth, ((struct param_p*) params)->classifier, ((struct param_p*) params)->pose_classifier,((struct param_p*) params)->out_width,(struct param_p*) params->out_height, (struct param_p*) params->min_area, Record, class_result, probs);
    std::cout << Record.size() <<std::endl;
    if (Record.size() > 0){
        args_p->data.clear();
        for(int r = 0 ; r < Record.size(); ++r){ 
           args_p->data.push_back(class_result[r]);
           //std::cout << class_result[r]<<std::endl;
           args_p->data.push_back(probs[r]);
           //std::cout << probs[r] <<std::endl;
           //std::cout << Record[r] << std::endl;
           args_p->data.push_back(Record[r].x);
           args_p->data.push_back(Record[r].y );
           args_p->data.push_back((Record[r].x + Record[r].width -1));
           args_p->data.push_back((Record[r].y + Record[r].height -1));
        }
   }
   *args_p->terminated = true;
}

void mask_on(cv::Mat &img, const cv::Mat binary_mat, int channel, uint8_t mask_value)
{
 int width = img.cols;
 int height = img.rows;
 ...
 {
    const uint8_t* bm = binary_mat.ptr<uint8_t>(i);
    uint8_t* im = channels[channel].ptr<uint8_t>(i);
...
int main(int, char**) try
{
    ...
    //std::string json_file = "model/deploy_ssd_300-symbol.json";
    //std::string param_file = "model/deploy_ssd_300-0240.params";
    //std::string json_file = "model/deploy_ssd_257-symbol.json";
    //std::string param_file = "model/deploy_ssd_257-0240.params";
    //BufferFile json_data(json_file);
    //BufferFile param_data(param_file);
    
    std::string model_file = "/home/louis/model/deploy_parse.prototxt";
    std::string trained_file = "/home/louis/model/fcn.caffemodel";
    std::string pose_model_file = "/home/louis/model/deploy_pose.prototxt";
    std::string pose_trained_file = "/home/louis/model/classify.caffemodel";
    //Classifier classifier(model_file, trained_file, 0, 0);
    //pose_Classifier pose_classifier(pose_model_file,pose_trained_file,0,0);
    ...
    int p_count = 0;
    int p_label = 0;
    cv::Mat mask(640,480,CV_8UC1,cv::Scalar(0));
    struct param_p args_p(model_file,trained_file,pose_model_file,pose_trained_file, &term, factor, isGpu_1) ;
    struct param_p args_p_2(model_file,trained_file,pose_model_file,pose_trained_file, &term,factor,isGpu_2) ;
    pthread_mutex_t mutex;
    //args_p.classifier = &classifier;
    //args_p.pose_classifier = &pose_classifier;
    args_p.out_width = out_width;
    args_p.out_height = out_height;
    args_p.min_area = min_area;
    //args_p.mask = mask;
    args_p_2.out_width = out_width;
    args_p_2.out_height = out_height;
    args_p_2.min_area = min_area;
    //args_p_2.mask = mask;
    // Parameters
   ...
   // int width = NETWORK_INPUT;
   // int height = NETWORK_INPUT;
    //int channels = 3;
   ...
   /* MXPredCreate((const char*)json_data.GetBuffer(),
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (auto&& sub : model.subdevices)
        {
            for (auto& queue : sub->queues)
            {
                try
                {
                    ...
                    {
                        model.upload_frame(std::move(f), mask);

                        if(SAVE) {
                            if(f.get_format() == RS2_FORMAT_Z16) {
                                std::ostringstream str;
                                str << "depth/" << f.get_frame_number()<<".raw";
                                std::string filename = str.str();
                                std::ofstream ofile;
                                ofile.open(filename, std::ofstream::out | std::ofstream::binary | std::ofstream::trunc);
                                ofile.write((const char*)f.get_data(),f.get_height()*f.get_width()*f.get_bytes_per_pixel());
                                ofile.close();
                            }
                            if(f.get_format() == RS2_FORMAT_RGB8) {
                                std::ostringstream str;
                                str << "rgb/" << f.get_frame_number()<<".raw";
                                std::string filename = str.str();
                                std::ofstream ofile;
                                ofile.open(filename, std::ofstream::out | std::ofstream::binary | std::ofstream::trunc);
                                ofile.write((const char*)f.get_data(),f.get_height()*f.get_width()*f.get_bytes_per_pixel());
                        ...
                        //std::vector<uchar> image_data3 = std::vector<uchar>(f.get_height() * f.get_width() * 3);
                        char *framedata = (char*)f.get_data();
                        /*for(int i = 0; i < f.get_width() *f.get_height(); ++ i) {
                            image_data3[3 * i + 2] = framedata[2 * i];
                            image_data3[3 * i + 1] = framedata[2 * i + 1];
                            image_data3[3 * i] = 0;
                        }*/
                       // const short *framedata_short = (const short*)f.get_data();
                       // std::vector<short> image_data_3(framedata_short, framedata_short + f.get_height() * f.get_width());

                        //cv::Mat img(f.get_height(), f.get_width(), CV_8UC3, image_data3.data());
                        //cv::Mat img = depth_2_rgb(framedata, f.get_width(), f.get_height());
                        
                        //std::cout << img.channels() << std::endl;
                        cv::Mat img_depth;
                        img_depth.create(f.get_height(), f.get_width(), CV_16UC1);
                        memcpy(img_depth.data, framedata, f.get_height() * f.get_width()*2);
                        //cv::Mat resize_img_depth;
                        //cv::Size size(f.get_width()/factor,f.get_height()/factor);
                        //cv::resize(img_depth,resize_img_depth,size);
                        
                        if(p_count == 120000)
                           p_count = 0;
                        pthread_mutex_lock(&mutex);

                        //if(std::abs(p_count - p_label) > 5){//every five pics
                        if(*args_p.terminated == true){
                             p_label = p_count;
                             if(args_p.data.size() > 0){
                                fixed_data = args_p.data;
                                mask = args_p.mask;
                             }
                             *args_p.terminated = false;
                             args_p.img_depth = img_depth;
                             //std::cout << "thread is not exist!" << std::endl;
                             pthread_create(&p, NULL, classify, (void*)&args_p);
                              
                             pthread_detach(p);
                        }
                       /* else if(*args_p_2.terminated == true){
                             p_label = p_count;
                             if(args_p_2.data.size()>0){
                                fixed_data = args_p.data;
                                mask = args_p_2.mask;
                              }
                             *args_p_2.terminated = false;
                             args_p_2.img_depth  = img_depth;
                             std::cout << "thread is not exist!" << std::endl;
                             pthread_create(&p2, NULL, classify, (void*)&args_p_2);
                             pthread_detach(p2);
                        }
                        }*/
                       
                        pthread_mutex_unlock(&mutex);
                        if(std::abs(p_label - p_count) > 60){
                        fixed_data.clear();
                        mask = cv::Mat((480,640,CV_8UC1,cv::Scalar(0)));
                        }
                        p_count++;
                        if(fixed_data.size() > 0){
                        //std::cout << args_p.mask << std::endl;
                        //do mask
                        //std::cout << args_p.mask.cols << " " << args_p.mask.rows << std::endl;
                        
                        std::cout<<"output coor: ";
                        for(int d =0; d<fixed_data.size();++d){
                         if(d%6 == 0){
                             std::cout<<std::endl;
                             std::cout << classes[(int)fixed_data[d]] << " ";
                         }
                         else
                             std::cout << fixed_data[d] << " ";
                        }
                        std::cout<<std::endl;
                        }
        ...
        for (auto kvp : layout)
        {
            ...
            model.streams[stream].data = fixed_data;
