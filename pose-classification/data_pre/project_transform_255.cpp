...
int main(){
   ...
   int count = 1;
    ...
    int num_length = 4000;
    int step = 3;
    std::string src_folder = "./lying/";
    std::string dest_folder = "../image-lying/";
    std::string jpg_folder = "./jpg-lying/";
    ...
    for (int j = 0;j < rectRecord.size();++j){
         ...
         cv::Mat inte_Mat;
         cv::Mat bg_Mat(out_height, out_width, CV_32SC1,cv::Scalar(0));
         img_depth(rectRecord[j]).convertTo(inte_Mat,CV_32S);
         Depth_Process(inte_Mat,left_cut_ratio,right_cut_ratio);
         inte_Mat.convertTo(inte_Mat,CV_32F);
         result_Reshape(inte_Mat,out_height,out_width,0.9);
         inte_Mat.convertTo(inte_Mat, CV_32S);
         ...
         fwrite(mydata, sizeof(uchar),  out_height*out_width, fp);
         fclose(fp);
         //free(mydata);
