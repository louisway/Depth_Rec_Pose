...

{
    inline void make_depth_histogram(uint8_t rgb_image[], const uint16_t depth_image[], int width, int height)
    {
        ...
        /*for (auto i = 0; i < width*height; ++i){
            ...
            rgb_image[i * 3 + 2] = depth_image[i] & 0xFF;
            rgb_image[i * 3 + 1] = (depth_image[i] & (0xFF << 8)) >> 8;
            rgb_image[i * 3 + 0] = 0;
        }*/
    }
    ...
    struct rect
    {
        ...
        bool contains(const float_2& p) const
        {
        ...
        rect adjust_ratio(float_2 size) const
        {
...
#include "third_party/stb_easy_font.h"
    ...
    class texture_buffer
    {
        ...
        rs2::frame last;
        void mask_on(uint8_t img_data[], cv::Mat &mask, int channel)
        {
             int size = mask.rows * mask.cols;
             int start = channel;
             for(int i = 0;i< mask.rows; ++i)
             {
                 const uint8_t* bm = mask.ptr<uint8_t>(i);
        ...
        void upload(const uint8_t * data, int width, int height, rs2_format format, cv::Mat &mask, int stride = 0, rs2_stream stream = RS2_STREAM_ANY)
        {
    ...
    public:
        ...
        float_2 xyz_to_xy(float x, float y, float z, GLfloat model[], GLfloat proj[], float vec_norm)
        {
        ...
        void upload(const rs2::frame& frame, cv::Mat &mask)
        {
            upload(static_cast<const uint8_t*>(frame.get_data()), frame.get_width(), frame.get_height(), frame.get_format(), mask,
        ...
        void show(const rect& r, float alpha, std::vector<float>& data) const
        {
            glEnable(GL_BLEND);
            ...
            */
            ...
            struct BBox {
                ...
                BBox(float x1, float y1, float x2, float y2, float score, float status) {
                    this->x1 = x1;
                    this->y1 = y1;
                    this->x2 = x2;
                    this->y2 = y2;
                ...
                float IoU(BBox &b) {
                    float X1 = std::max(x1, b.x1);
                    float Y1 = std::max(y1, b.y1);
                    float X2 = std::min(x2, b.x2);
                    float Y2 = std::min(y2, b.y2);
                    float interArea = (X2 - X1) * (Y2 - Y1);
                    float unionArea = area + b.area - interArea;
                    return interArea / area;
                }

                bool operator < (const BBox &b) const {
                    return (score - b.score) > 0; //BIGGER FRONT
                }
            };

            std::vector<BBox> BBoxes;
            for(int i = 0; i < data.size(); i += 6) {
                if(data[i] >= 0 && data[i + 1] > 0.25) {
                    float x1 = (data[i + 2]/640) * (float)r.w + (float)r.x;
                    float y1 = (data[i + 3]/480) *(float)r.h + (float)r.y;
                    float x2 = (data[i + 4]/640)*(float)r.w + (float)r.x;
                    float y2 = (data[i + 5]/480)*(float)r.h + (float)r.y;
                    float score = data[i + 1];
                    BBoxes.push_back(BBox(x1, y1, x2, y2, data[i + 1],data[i]));
                }
            }
            if(BBoxes.empty()) {
                return;
            ...
            //std::cout << BBoxes.begin()->score << ", " << (BBoxes.begin() + BBoxes.size() - 1)->score << std::endl;
            ...
            for(std::vector<BBox>::iterator it = BBoxes.begin(); it != BBoxes.end(); ++ it) {
                if(it -> bestBox) {
                    ...
                    glEnd();
                    glFlush();
                    //std::cout << "output is here" <<std::endl;
                    //printf("\t%f, %f, %f, %f, %f\n", it->score, it->x1, it->y1, it->x2, it->y2);
                    std::stringstream st;
                    st << classes[it->status] << " ,prob: " << it->score;
                    draw_text(x1, y1, st.str().c_str());
                    
                    std::vector<BBox>::iterator j = it + 1;
                    while(j != BBoxes.end()) {
                        std::vector<BBox>::iterator jNext = j + 1;
                        if(it->IoU(*j) > 0.3) {
                            //j->bestBox = false;
                        }
                        j = jNext;
